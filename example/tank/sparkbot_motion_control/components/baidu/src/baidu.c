...
#define BAIDUBCE_URL            "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/ernie-lite-8k?access_token=24.b77d1c4df6c0f9d8937467d06fe91b06.2592000.1731316960.282335-115842376"
...
esp_err_t baidu_asr_send_audio(const int16_t *audio, size_t len, size_t total_len)
{
    if (!g_baidu_asr_http_handle) {
        esp_http_client_config_t config = {
            .url = "http://vop.baidu.com/server_api?dev_pid=1537&cuid=123456PHP&token=24.d703dcd2916c9993fcb465e169533abb.2592000.1731324425.282335-115843676",
            .method = HTTP_METHOD_POST,
            .skip_cert_common_name_check = true,
            .buffer_size = 1460,
            .timeout_ms = 30000,
...
esp_err_t baidu_tts_send_text(const char *text)
{
    ...
    #define BAIDU_TTS_FORMAT "tex=%s&tok=24.d703dcd2916c9993fcb465e169533abb.2592000.1731324425.282335-115843676&cuid=12345postman&ctp=1&lan=zh&spd=6&pit=5&vol=8&per=4100&aue=6&="

    char *payload = NULL;
    // Create JSON payload with model, max tokens, and content
    asprintf(&payload, BAIDU_TTS_FORMAT, text);
